...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.ulvxpe7.mongodb.net/?retryWrites=true&w=majority`;
...
async function run() {
  try {
    ...
    const cartCollecction = client.db("BistroDB").collection("carts");
...
const verifyAdmin = async (req, res, next) => {
  ...
  app.post("/menu", verifyToken, verifyAdmin, async(req, res) => {
      const item = req.body; 
      const result = await menuCollecction.insertOne(item); 
      res.send(result); 
  }) 
  ...
  app.patch("/menu/:id", async (req, res) => {
      ...
      const result = await menuCollecction.updateOne(filter, updatedDoc);
      res.send(result); 
  })
  ...
  app.delete("/menu/:id", verifyToken, verifyAdmin, async (req, res) => {
    ...
    console.log(id);
    const query = { _id: new ObjectId(id) };
    const result = await menuCollecction.deleteOne(query); 
    console.log(result);
    res.send(result); 
 })
    ...
    app.get("/menu/:id", async (req, res) => {
        const id = req.params.id; 
        const query = { _id: new ObjectId(id) };
        const result = await menuCollecction.findOne(query); 
        res.send(result); 
    })
    ...
    app.post("/carts", async (req, res) => {
      ...
      const result = await cartCollecction.insertOne(cartItem);
      res.send(result);
    });
    ...
    app.get("/carts", async (req, res) => {
      ...
      const query = { email: email };
      const result = await cartCollecction.find(query).toArray();
      res.send(result);
    });
    ...
    app.delete("/carts/:id", async (req, res) => {
      const id = req.params.id;
      const query = { _id: new ObjectId(id) };
      const result = await cartCollecction.deleteOne(query);
      res.send(result);
    });
